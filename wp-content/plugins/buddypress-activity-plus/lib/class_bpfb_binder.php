...
EOFontIconCSS;
	...
	function ajax_update_activity_contents () {
		...
		if ($bpfb_code) {
			...
			$datepicker = !empty($_POST['datepicker']) ? $_POST['datepicker'] : '';
			$aid = $gid ?
				groups_post_update(array('content' => $content, 'group_id' => $gid))
				:
				bp_activity_post_update(array('content' => $content, 'datepicker'=>$datepicker))
			;
			global $blog_id;
			bp_activity_update_meta($aid, 'bpfb_blog_id', $blog_id);
			bp_activity_update_meta($aid, 'datepicker', $datepicker);
		}
		if ($aid) {
			ob_start();
