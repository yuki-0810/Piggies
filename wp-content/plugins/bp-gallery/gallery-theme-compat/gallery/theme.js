...
//j(".activity audio").each(function (){
   // alert("yep");
    j(".activity-list audio").css({width:'310px'}); 
//});
/**
...
j(document).on('click','.gallery-actions a.upload, #upload_media_link,#gallery_media_upload',function() {
...
//cover upload form
j(document).on('click', '#gallery_gallery_cover_upload', function() {
        var url = j( this ).attr( 'href' );
        var gid = helper.get_var_in_url( url, 'gallery_id' );
	var nonce = helper.get_nonce( url );
	j.post( ajaxurl, {
...
j(document).on('click', '#gallery_upload_buttons_for_activity a', function() {
        var el=j(this);
        var form = j("#gallery_media_upload_form");//.remove();
		...
		function(response){
                       ...
                       j(container).fadeOut(200,
                            function() {
                                ...
                                j("#gallery_media_upload_form").data('media-type', type);
                                if(helper.is_flash_enabled())
...
j(document).on('click', '#save_gallery_cover', function(){
    if(!helper.is_flash_enabled())
        return ;
     var swfu = j.swfupload.getInstance('#swfupload-control');
 ...
 */
    j(document).on('click','#bulk_upload_media_submit',function() {
    ...
    //add from web
    j( document ).on( 'click', '#gallery_save_from_web', function(){
    ...
    j( document ).on( 'click', '#activity_upload_media_submit', function() {
...
function gallery_activate_player_on_organize(){
    ...
    jQuery('div.media-cover video,div.media-cover audio').mediaelementplayer({audioWidth:300});
 }
