...
class BP_Gallery_Media {
		...
		if ( $this->id ) {
			$sql = $wpdb->prepare( "UPDATE {$bp->gallery->table_media_data} SET
					...
					date_updated = %s
                                        WHERE
					id = %d
                                        ", $this->gallery_id, $this->title, $this->slug, $this->description, $this->type, $this->user_id, $this->is_remote, $this->status, $this->sort_order, $this->remote_url, $this->local_orig_path, $this->local_mid_path, $this->local_thumb_path, $this->date_updated, $this->id
			);
		} else {
			$sql = $wpdb->prepare( "INSERT INTO {$bp->gallery->table_media_data} (
					...
					%d, %s, %s, %s, %s, %d, %d, %s,%d, %s, %s, %s, %s, %s
                                    )", $this->gallery_id, $this->title, $this->slug, $this->description, $this->type, $this->user_id, $this->is_remote, $this->status, $this->sort_order, $this->remote_url, $this->local_orig_path, $this->local_mid_path, $this->local_thumb_path, $this->date_updated
			);
		}
	...
	public static function delete_for_gallery( $gallery_id ) {
		global $bp, $wpdb;
	...
	public static function get_all( $status_filter = array( 'public' ), $type = null, $gallery_id = null, $user_id = null, $limit = null, $page = null, $order_by = "sort_order", $sort_order = null ) {
		global $wpdb, $bp;
...
//helper function for preparing the order by sql
	public static function get_orderby_sql( $order_by, $order = null ) {
		global $wpdb;
	 ...
	 */
	public static function media_exists( $slug, $gallery_id ) {
		global $wpdb, $bp;
		if ( ! $slug )
			return false;
	 ...
	 * @return <type>
	 */
	public static function get_id_from_slug( $slug, $gallery_id ) {
		return self::media_exists( $slug, $gallery_id );
	}

	public static function check_slug( $slug, $gallery_id ) {
		global $wpdb, $bp;
	...
	public static function get_slug( $media_id ) {
		global $wpdb, $bp;
...
//need to be improved and code to be fine tuned, it is not good practice
	public static function get_adjacent_gallery_media( $previous = true, $status_filter = null, $media = null ) {
...
//for all gallery below this gallery update order
	 ...
	 * @return <type> 
	 */
	public static function get_order( $gallery_id ) {
//get the order of current gallery based on the owner object type,owner object id and the gallry type
	 ...
	 */
	public static function get_media_count( $gallery_id ) {
		global $wpdb, $bp;
		$where_conditions = array();
...
//force access level
	...
	public static function gallery_has_media( $gallery ) {
		return BP_Gallery_Media::get_media_count( $gallery->id );
	}

	public static function get_first_media_id( $gallery ) {
