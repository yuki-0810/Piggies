...
function gallery_get_media_comment_count($media_id,$show_hidden=true){
  ...
  /* Alter the query based on whether we want to show activity item comments in the stream like normal comments or threaded below the activity */
    ...
    $total_activities = $wpdb->get_var(  "SELECT count(a.id) {$from_sql} {$where_sql} "  );
...
$activity_action=sprintf( __( "%s uploaded %d %s to %s" , 'bp-gallery'),bp_core_get_userlink( $bp->loggedin_user->id ),$count,$type,'<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>'.":"  );
...
$activity_action=sprintf( __( "%s uploaded %d %s to %s" , 'bp-gallery'),bp_core_get_userlink( $bp->loggedin_user->id ),count($published_media),$type,'<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>'.":"  );
...
//if($gallery)
    ...
    $activity_action=sprintf( __( "%s uploaded %s %s to %s" , 'bp-gallery'),bp_core_get_userlink( $media->user_id ),$a_or_an,$type,'<a href="' . bp_get_gallery_permalink( $gallery ) . '">' . esc_attr( $gallery->title ) . '</a>'.":"  );
...
add_filter("bp_legacy_theme_ajax_querystring","bp_gallery_filter_activity_for_user_querystring",12,7);//for template pack
