...
function bp_gallery_get_possible_upload_errors($allowed_size=null){
    $upload_errors = array(
                        ...
                        UPLOAD_ERR_INI_SIZE =>  sprintf( __('Your file size was bigger than the maximum allowed file size of: %s', 'bp-gallery' ), $allowed_size ),
                        UPLOAD_ERR_FORM_SIZE => sprintf(  __( 'Your file was bigger than the maximum allowed file size of: %s', 'bp-gallery' ), $allowed_size ),
...
if ( 4 !== $file['file']['error'] ) {
	...
	if ( !$checked_upload = bp_core_check_avatar_upload($file) )
        $error= sprintf( __("There was an error uploading the file.Error details: %s", "bp-gallery"), $upload_errors[$file['file']['error']] );

	/* check for maximum upload size*/
    if ( $checked_upload && !$checked_size = gallery_check_file_size($file['file']) )
		$error = sprintf( __('The file you uploaded is too big. Please upload a file under %s', 'bp-gallery'), $allowed_size);


	//check for file type:allowed are jpeg/jpg/png/gif

    if ( $checked_upload && $checked_size && !$checked_type =bp_gallery_is_media_of_type($file['file'],"photo") ) {
		$error = sprintf( __( 'Please upload only %s %s', 'bp-gallery' ), gallery_get_verbose_explanation_for_extension($gallery->gallery_type), $gallery->gallery_type );
	}
//remember we are doing progressive checking here
...
function gallery_media_process_image($media){
    ...
    foreach($settings as $size_type=>$dimensions){
        $resized_image =bp_gallery_image_resize($media['file'], $settings[$size_type]['width'], $settings[$size_type]['height'],$is_crop );//create a mid size thumbnail
 ...
 */
function bp_gallery_image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 90 ) {
