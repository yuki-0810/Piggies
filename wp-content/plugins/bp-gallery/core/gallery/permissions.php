...
function gallery_user_can_upload($gallery){
    ...
    $can_do=false;
    if(is_super_admin()||gallery_is_user_admin(get_current_user_id(),$gallery->id))
        $can_do=true;
...
if(is_user_logged_in())
    $access[]="loggedin";
if(bp_is_active('friends')&&('is_friend'==BP_Friends_Friendship::check_is_friend(get_current_user_id(),$user_id)))
    $access[]="friendsonly";
...
if($gallery->status=="friendsonly" && (class_exists("BP_Friends_Friendship")&&'is_friend'==BP_Friends_Friendship::check_is_friend(get_current_user_id(),bp_displayed_user_id())))
return true;
