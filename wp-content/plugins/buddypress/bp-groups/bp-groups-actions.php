...
function groups_action_create_group() {
	...
	if ( isset( $_POST['save'] ) ) {
		...
		if ( 'group-details' == bp_get_groups_current_create_step() ) {
			if ( empty( $_POST['group-name'] ) || empty( $_POST['group-desc'] ) || empty( $_POST['university-teacher'] ) || !strlen( trim( $_POST['group-name'] ) ) || !strlen( trim( $_POST['group-desc'] ) ) ) {
				bp_core_add_message( __( '必須事項を記入してください。', 'buddypress' ), 'error' );
