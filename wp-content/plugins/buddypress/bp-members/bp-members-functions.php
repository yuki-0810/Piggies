...
function bp_core_add_validation_error_messages( WP_Error $errors, $validation_results ) {
	if ( ! empty( $validation_results['invalid'] ) ) {
		$errors->add( 'user_email', __( 'Eメールアドレスを確認してください', 'buddypress' ) );
	}
...
function bp_core_validate_user_signup( $user_name, $user_email ) {
	...
	// minus the multisite-specific restrictions on user_login.
		...
		if ( empty( $user_name ) ) {
			$errors->add( 'user_name', __( 'ユーザー名を入力してください', 'buddypress' ) );
		}

		// User name can't be on the blacklist.
		$illegal_names = get_site_option( 'illegal_names' );
		if ( in_array( $user_name, (array) $illegal_names ) ) {
			$errors->add( 'user_name', __( 'このユーザー名は許可されていません', 'buddypress' ) );
		}

		// User name must pass WP's validity check.
		if ( ! validate_username( $user_name ) ) {
			$errors->add( 'user_name', __( 'ユーザー名は半角英数字で入力してください', 'buddypress' ) );
		}

		// Minimum of 4 characters.
		if ( strlen( $user_name ) < 4 ) {
			$errors->add( 'user_name',  __( 'ユーザー名は最低４文字以上で入力してください', 'buddypress' ) );
		}
		...
		if ( $match[0] == $user_name ) {
			$errors->add( 'user_name', __( '少なくとも一文字アルファベットを入れてください。', 'buddypress' ) );
		}
