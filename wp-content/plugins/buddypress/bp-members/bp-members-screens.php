...
function bp_core_screen_signup() {
	...
	if ( !bp_get_signup_allowed() ) {
		...
		if ( empty( $_POST['signup_password'] ) || empty( $_POST['signup_password_confirm'] ) )
			$bp->signup->errors['signup_password'] = __( 'パスワードを入力してください', 'buddypress' );

		// Check that the passwords match.
		if ( ( !empty( $_POST['signup_password'] ) && !empty( $_POST['signup_password_confirm'] ) ) && $_POST['signup_password'] != $_POST['signup_password_confirm'] )
			$bp->signup->errors['signup_password'] = __( '入力されたパスワードが確認パスワードと違います', 'buddypress' );
			...
			if ( isset( $_POST['signup_profile_field_ids'] ) && !empty( $_POST['signup_profile_field_ids'] ) ) {
				...
				foreach ( (array) $profile_field_ids as $field_id ) {
					...
					if ( xprofile_check_is_required_field( $field_id ) && empty( $_POST[ 'field_' . $field_id ] ) && ! bp_current_user_can( 'bp_moderate' ) )
						$bp->signup->errors['field_' . $field_id] = __( '必須項目です', 'buddypress' );
				}
...
function bp_core_screen_activation() {
	...
	if ( ! empty( $key ) ) {
		...
		bp_core_add_message( __( '有効化が完了しました！', 'buddypress' ) );
